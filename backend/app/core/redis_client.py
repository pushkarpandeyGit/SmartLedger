...
try:
    ...
    if settings.REDIS_URL:
        # Connect to Upstash Cloud Redis using URL
    ...
    print("[Redis Cache] Connected to Redis server successfully. In-memory caching active.")
except Exception as e:
    _redis_available = False
    logger.warning(f"[Redis Notice] Redis offline ({e}). Graceful memory cache fallback active.")
    print(f"[Redis Notice] Redis offline ({e}). Graceful memory cache fallback active.")
